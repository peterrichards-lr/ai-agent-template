...
| **`documentation-review`** | Governs auditing an existing documentation set: measure before diagnosing, inspect the rendered output rather than the source, check documents against the code, and fix routing before restructuring. Sibling to `documentation`, which covers hygiene after a change. |
...
| `SECURITY.md` | Vulnerability disclosure policy, secret-scanning gates and the SHA-pinning rationale. Its opening line and its reporting address are both substituted, so the delivered policy names your project and an address a reporter can actually reach. It reuses `--conduct-email` rather than adding a second required flag: a newly bootstrapped project has one maintainer inbox, and a policy naming no destination fails the only job it has. | `<CONDUCT_EMAIL_PLACEHOLDER>` (`--conduct-email`) |
